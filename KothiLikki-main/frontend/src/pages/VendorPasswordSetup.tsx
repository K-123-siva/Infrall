...
import { Lock, Mail, Briefcase, CheckCircle, AlertCircle } from 'lucide-react';
import api from '../api';

export default function VendorPasswordSetup() {
  ...
  const verifyToken = async () => {
    ...
    try {
      const { data } = await api.get(`/account-management/verify-token?token=${token}&type=vendor`);
      setValid(data.valid);
      setAccountInfo(data);
    } catch (error: any) {
  ...
  if (!valid || error) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', padding: 20 }}>
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', padding: 20 }}>
        <div style={{ background: '#fff', borderRadius: 16, padding: 40, maxWidth: 500, width: '100%', textAlign: 'center' }}>
          ...
          <p style={{ color: '#64748b', marginBottom: 24 }}>
            Your vendor account is now active. Redirecting to login page...
          </p>
        </div>
      </div>
    );
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 40, maxWidth: 500, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: '#64748b', fontSize: 14 }}>
            Welcome to INFRAALL Vendor Portal
          </p>
        </div>

        {accountInfo && (
          <div style={{ background: '#f0fdf4', padding: 16, borderRadius: 8, marginBottom: 24, border: '1px solid #bbf7d0' }}>
            {accountInfo.businessName && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <Briefcase size={16} color="#059669" />
                  <span style={{ fontSize: 14, color: '#065f46', fontWeight: 600 }}>Business Name</span>
                </div>
                <div style={{ fontSize: 16, color: '#1e293b', fontWeight: 600 }}>{accountInfo.businessName}</div>
              </div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <Mail size={16} color="#059669" />
              <span style={{ fontSize: 14, color: '#065f46', fontWeight: 600 }}>Account Email</span>
            </div>
        ...
        <div style={{ marginTop: 24, textAlign: 'center', fontSize: 13, color: '#64748b' }}>
          ...
          <a href="/vendor/login" style={{ color: '#10b981', fontWeight: 600 }}>
            Vendor Portal
          </a>
        </div>
      </div>
